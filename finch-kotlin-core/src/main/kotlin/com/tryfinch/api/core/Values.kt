...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String = asString() ?: throw FinchInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw FinchInvalidDataException("`$name` is not set")
            is JsonNull -> throw FinchInvalidDataException("`$name` is null")
