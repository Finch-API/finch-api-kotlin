...
class WebhookEvent private constructor(
    ...
    fun accountUpdated(): AccountUpdateEvent? = accountUpdated

    fun jobCompletion(): JobCompletionEvent? = jobCompletion

    fun companyUpdated(): CompanyEvent? = companyUpdated

    fun directory(): DirectoryEvent? = directory

    fun employment(): EmploymentEvent? = employment

    fun individual(): IndividualEvent? = individual

    fun payment(): PaymentEvent? = payment

    fun payStatement(): PayStatementEvent? = payStatement
    ...
    companion object {
        ...
        fun ofAccountUpdated(accountUpdated: AccountUpdateEvent) = WebhookEvent(accountUpdated = accountUpdated)

        fun ofJobCompletion(jobCompletion: JobCompletionEvent) = WebhookEvent(jobCompletion = jobCompletion)

        fun ofCompanyUpdated(companyUpdated: CompanyEvent) = WebhookEvent(companyUpdated = companyUpdated)

        fun ofDirectory(directory: DirectoryEvent) = WebhookEvent(directory = directory)
        ...
        fun ofPayment(payment: PaymentEvent) = WebhookEvent(payment = payment)

        fun ofPayStatement(payStatement: PayStatementEvent) = WebhookEvent(payStatement = payStatement)
    }

    /**
     * An interface that defines how to map each variant of [WebhookEvent] to a value
     * of type [T].
     */
    interface Visitor<out T> {
         ...
         *
         * An instance of [WebhookEvent] can contain an unknown variant if it was
    ...
    internal class Deserializer : BaseDeserializer<WebhookEvent>(WebhookEvent::class) {
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): WebhookEvent {
          ...
          tryDeserialize(node, jacksonTypeRef<AccountUpdateEvent>()){ it.validate() }?.let {
              return WebhookEvent(accountUpdated = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<JobCompletionEvent>()){ it.validate() }?.let {
              return WebhookEvent(jobCompletion = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<CompanyEvent>()){ it.validate() }?.let {
              return WebhookEvent(companyUpdated = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<DirectoryEvent>()){ it.validate() }?.let {
              return WebhookEvent(directory = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<EmploymentEvent>()){ it.validate() }?.let {
              return WebhookEvent(employment = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<IndividualEvent>()){ it.validate() }?.let {
              return WebhookEvent(individual = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<PaymentEvent>()){ it.validate() }?.let {
              return WebhookEvent(payment = it, _json = json)
          }
          tryDeserialize(node, jacksonTypeRef<PayStatementEvent>()){ it.validate() }?.let {
    ...
    internal class Serializer : BaseSerializer<WebhookEvent>(WebhookEvent::class) {
        ...
        override fun serialize(value: WebhookEvent, generator: JsonGenerator, provider: SerializerProvider) {
          when {
