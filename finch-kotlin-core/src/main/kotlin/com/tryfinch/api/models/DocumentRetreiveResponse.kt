...
class DocumentRetreiveResponse private constructor(
    ...
    fun validate(): DocumentRetreiveResponse =
        apply {
            if (validated) {
              return@apply
            }
    ...
    /**
     * An interface that defines how to map each variant of [DocumentRetreiveResponse]
    ...
    interface Visitor<out T> {
         ...
         *
         * An instance of [DocumentRetreiveResponse] can contain an unknown variant if it
    ...
    internal class Deserializer : BaseDeserializer<DocumentRetreiveResponse>(DocumentRetreiveResponse::class) {
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): DocumentRetreiveResponse {
          ...
          when (type) {
              "w4_2020" -> {
                  tryDeserialize(node, jacksonTypeRef<W42020>()){ it.validate() }?.let {
                      return DocumentRetreiveResponse(w42020 = it, _json = json)
                  }
              }
              "w4_2005" -> {
                  tryDeserialize(node, jacksonTypeRef<W42005>()){ it.validate() }?.let {
    ...
    internal class Serializer : BaseSerializer<DocumentRetreiveResponse>(DocumentRetreiveResponse::class) {

        override fun serialize(value: DocumentRetreiveResponse, generator: JsonGenerator, provider: SerializerProvider) {
          when {
